...
def text_reply(msg):
    print(msg['Text'])
    itchat.send('这是我的测试，可尽情调戏', msg['FromUserName'])
...
def group_reply_text(msg):
    ...
    #     return
    if not chatroom_id in '@0b73ba438fdede17c40be4bc91367e5e':
        return
    ...
    if msg['Type'] == TEXT:
        ...
        content = msg['Text']
    print('准备')
    #itchat.send(msg['FileName'], chatroom_id)
    #itchat.send('%s\n%s\n%s' % (username, msg['Text'], msg['Url']), chatroom_id)
...
def group_reply_media(msg):
    ...
    # 消息并不是来自于需要同步的群
    if not chatroom_id in '@0b73ba438fdede17c40be4bc91367e5e': #chatroom_ids:
        return
    ...
    itchat.send('@%s@%s' % ({'Picture': 'img', 'Video': 'vid'}.get(msg['Type'], 'fil'), msg['FileName']), chatroom_id)
